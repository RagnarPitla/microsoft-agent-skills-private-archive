...
test("a promoted skill missing from the docs index fails", (t) => {
  const dir = scratchRepo(t, (s) => {
    s.edit("docs/README.md", (text) => text.replace(/^- \[write-a-skill\].*$/m, "- write-a-skill"));
  });
...
test("a docs page missing one of its four sections fails", (t) => {
  ...
  failsWith(validate(dir), /is missing the "It's working if" section/);
});
...
test("a non-promoted skill that leaks into public surface area fails", (t) => {
  ...
  assert.match(result.output, /beta-skill: in non-promoted bucket "in-progress" but listed in docs\/README\.md/);
});
...
test("a malformed verified_on fails", (t) => {
  ...
  failsWith(validate(dir), /`verified_on` must be YYYY-MM-DD/);
});
...
test("a stale verified_on warns but does not fail", (t) => {
  // Deliberately not an error. Staleness is a maintenance queue, and failing an
  // unrelated pull request over it is how a check gets routed around.
  const dir = scratchRepo(t, (s) => {
...
test("a version bump with no changelog entry fails", (t) => {
  const dir = scratchRepo(t, (s) => {
    s.edit("package.json", (text) => text.replace(/"version": ".*"/, '"version": "9.9.9"'));
  });
  failsWith(validate(dir), /CHANGELOG\.md has no section for the current version 9\.9\.9/);
});
